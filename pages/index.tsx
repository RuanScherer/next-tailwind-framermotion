...
const Card: React.FC<CardProps> = ({ href, imgSrc, layoutId}) => {
  return (
    <Link href={href}>
      <div className="relative h-20 md:h-40 bg-gray-200 rounded-xl cursor-pointer hover:bg-gray-100 transition">
        <motion.img src={`/img/${imgSrc}`} className="w-full h-full" layoutId={layoutId}/>
      </div>
    </Link>
  )
}
